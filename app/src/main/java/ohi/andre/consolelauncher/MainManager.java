...
import ohi.andre.consolelauncher.managers.RootixLocationManager;
...
public class MainManager {
    ...
    protected MainManager(LauncherActivity c) {
        ...
        keeperServiceRunning = XMLPrefsManager.getBoolean(Behavior.rootix_notification);
        ...
        receiver = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                if (action.equals(ACTION_EXEC)) {
                    ...
                    Tuils.sendOutput(context, "Lat: " + intent.getDoubleExtra(RootixLocationManager.LATITUDE, 0) + "; Long: " + intent.getDoubleExtra(RootixLocationManager.LONGITUDE, 0));
                    RootixLocationManager.instance(context).rm(location.ACTION_LOCATION_CMD_GOT);
                }
            }
        };
...
//    command manager
    private static final Pattern DOT_AI_TRIGGER = Pattern.compile("^\\.ai\\s*(.*)$", Pattern.CASE_INSENSITIVE);

    public void onCommand(String input, String alias, boolean wasMusicService) {
        input = Tuils.removeUnncesarySpaces(input);

        //  ".ai" / ".AI" is a convenience shortcut for the "ai" command
        Matcher aiTriggerMatcher = DOT_AI_TRIGGER.matcher(input.trim());
        if(aiTriggerMatcher.matches()) {
            String aiText = aiTriggerMatcher.group(1);
            input = "ai" + (aiText.length() > 0 ? (Tuils.SPACE + aiText) : Tuils.EMPTYSTRING);
        }
    ...
    public void destroy() {
        mainPack.destroy();
        RootixLocationManager.disposeStatic();
...
//
    ...
    private class RootixCommandTrigger implements CmdTrigger {
