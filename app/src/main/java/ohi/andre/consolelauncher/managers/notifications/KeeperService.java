...
//    public static final String BIND_NODE = "binding", ID_ATTRIBUTE = "id", APP_ATTRIBUTE = "pkg";
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        if(startId == 1 || startId == 0) {
            ...
            title = XMLPrefsManager.get(Behavior.rootix_notification_title);
            subtitle = XMLPrefsManager.get(Behavior.rootix_notification_subtitle);
            clickCmd = XMLPrefsManager.get(Behavior.rootix_notification_click_cmd);
            inputFormat = XMLPrefsManager.get(Behavior.input_format);
            showHome = XMLPrefsManager.getBoolean(Behavior.rootix_notification_click_showhome);
            inputColor = XMLPrefsManager.getColor(Behavior.rootix_notification_input_color);
            timeColor = XMLPrefsManager.getColor(Behavior.rootix_notification_time_color);
            prefix = XMLPrefsManager.get(Ui.input_prefix);
            upDown = XMLPrefsManager.getBoolean(Behavior.rootix_notification_lastcmds_updown);
            suPrefix = XMLPrefsManager.get(Ui.input_root_prefix);

            priority = XMLPrefsManager.getInt(Behavior.rootix_notification_priority);
            ...
            int lastCmdSize = XMLPrefsManager.getInt(Behavior.rootix_notification_lastcmds_size);
            if(lastCmdSize > 0) {
