...
import com.rootix.launcher.commands.Command;
import com.rootix.launcher.commands.CommandGroup;
import com.rootix.launcher.commands.CommandTuils;
import com.rootix.launcher.commands.main.MainPack;
import com.rootix.launcher.commands.main.raw.location;
import com.rootix.launcher.commands.main.specific.RedirectCommand;
import com.rootix.launcher.managers.AiManager;
...
import com.rootix.launcher.managers.HTMLExtractManager;
import com.rootix.launcher.managers.RssManager;
import com.rootix.launcher.managers.TerminalManager;
import com.rootix.launcher.managers.ThemeManager;
import com.rootix.launcher.managers.TimeManager;
import com.rootix.launcher.managers.TuiLocationManager;
import com.rootix.launcher.managers.music.MusicManager2;
import com.rootix.launcher.managers.music.MusicService;
import com.rootix.launcher.managers.notifications.KeeperService;
import com.rootix.launcher.managers.xml.XMLPrefsManager;
import com.rootix.launcher.managers.xml.options.Behavior;
import com.rootix.launcher.managers.xml.options.Theme;
import com.rootix.launcher.tuils.BusyBoxInstaller;
import com.rootix.launcher.tuils.PrivateIOReceiver;
import com.rootix.launcher.tuils.StoppableThread;
import com.rootix.launcher.tuils.Tuils;
import com.rootix.launcher.tuils.interfaces.CommandExecuter;
import com.rootix.launcher.tuils.interfaces.OnRedirectionListener;
import com.rootix.launcher.tuils.interfaces.Redirectator;
import com.rootix.launcher.tuils.libsuperuser.Shell;
...
public class MainManager {
    ...
    private final String COMMANDS_PKG = "com.rootix.launcher.commands.main.raw";
...
//
    ...
    private class AiTrigger implements CmdTrigger {

        final Pattern aiPattern = Pattern.compile("^\\.ai(?:\\s+(.*))?$", Pattern.CASE_INSENSITIVE | Pattern.DOTALL);

        @Override
        public boolean trigger(final MainPack info, final String input) throws Exception {
            Matcher m = aiPattern.matcher(input.trim());
            if (!m.matches()) return false;

            String query = m.group(1);
            if (query == null || query.trim().length() == 0) {
                Tuils.sendOutput(mContext, "Usage: .ai <your question>\nExample: .ai who are you", TerminalManager.CATEGORY_OUTPUT);
                return true;
            }

            final String finalQuery = query.trim();

            //  ".ai open <app>" -> just launch the app directly, no need to ask a model for this
            if (finalQuery.toLowerCase().startsWith("open ")) {
                String appName = finalQuery.substring(5).trim();
                if (appName.length() > 0) {
                    AppsManager.LaunchInfo li = appsManager.findLaunchInfoWithLabel(appName, AppsManager.SHOWN_APPS);
                    if (li != null) {
                        performLaunch(info, li, input);
                    } else {
                        Tuils.sendOutput(mContext, "Couldn't find an app called \"" + appName + "\" to open", TerminalManager.CATEGORY_OUTPUT);
                    }
                    return true;
                }
            }

            Tuils.sendOutput(mContext, "Thinking...", TerminalManager.CATEGORY_OUTPUT);
            ...
            new StoppableThread() {
                ...
                public void run() {
                    ...
                    if (!Tuils.hasInternetAccess()) {
                        Tuils.sendOutput(Color.RED, mContext, "No internet connection");
                        return;
                    }

                    AiManager.ask(info.client, finalQuery, new AiManager.AiCallback() {
                        @Override
                        public void onSuccess(String model, String response) {
                            Tuils.sendOutput(mContext, "[" + model + "]\n" + response, TerminalManager.CATEGORY_OUTPUT);
                        }

                        @Override
                        public void onError(String error) {
                            Tuils.sendOutput(Color.RED, mContext, "All AI models failed, try again in a bit.\n" + error);
                        }
                    });
                }
            }.start();
